			       ...
			       struct light_state_t const *state)
{
    ...
    int brightness = is_lit(state);

	fdn = open("/sys/class/gpio/gpio169/value", O_RDWR);
	
    if (fdn >= 0) {
        char buffer[20];
        int bytes = sprintf(buffer, "%d", brightness);
        int amt = write(fdn, buffer, bytes);
        close(fdn);
        ...
        if (already_warned == 0) {
            LOGE("write_int failed to open LED PH02\n");
            already_warned = 1;
        }
		pthread_mutex_unlock(&g_lock);
        return -errno;
...
static int set_light_notification2(struct light_device_t *dev,
			       struct light_state_t const *state)
{
    static int already_warned;
    int err = 0;
	...
	fdn = open("/sys/class/gpio/gpio201/value", O_RDWR);
    ...
    if (fdn >= 0) {
        ...
        if (already_warned == 0) {
            LOGE("write_int failed to open LED PI00\n");
            already_warned = 1;
        }
		pthread_mutex_unlock(&g_lock);
        return -errno;
		       ...
		       struct hw_device_t **device)
{
	...
	{
		set_light = set_light_notification2; // AXON SYSTEM
	}
	else
		return -EINVAL;
